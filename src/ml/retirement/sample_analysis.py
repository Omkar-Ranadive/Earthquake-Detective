...
def extract_sub_ids(file, naive=False):
    """
    ...
    with open(file, 'r') as f:
        for index, line in enumerate(f.readlines()):
            ...
            if index > 2:
                ...
                elif naive and float(info[5]) > threshold:
                    sub_ids.append(info[0])

    return sub_ids


def naive_selection(path):
    """
    This function selects subject ids without consideration of the reliability scores
    i.e., all users are given equal weightage

    Returns (list): List of selected ids
    ...
    with open(file, 'r') as f:
        for index, line in enumerate(f.readlines()):
            ...
            if index > 2:
                info = line.split(",")
                if float(info[5]) >= threshold:
                    sub_ids.append(info[0])

    print("Selected {} ids out of {}".format(len(sub_ids), index+1))
...
def select_sub_ids(sub_ids, classifications):

    sub_stats = gen_subids_stats(sub_ids, classifications)
    selected_ids = []
    for sub, users in sub_stats.items():
        out = selection_criteria_1(users, label_of_interest=0)

        if out == 1:
            selected_ids.append(sub)
...
if __name__ == '__main__':
    ...
    for file in os.listdir(folder):
        ...
        select_sub_ids(all_ids, classifications='earthquake-detective-classifications.csv')
        ...
        common_ids = set(all_ids).intersection(set(naive_ids))
